...
const D: usize = 2;
type PGConfig = PoseidonGoldilocksConfig;
...
impl ConsecutiveProduct {
    pub fn new(circuit_builder: &mut CircuitBuilder<BaseField, D>, n_factors: usize) -> Self {
        ...
        let product_targets = build_prod_circuit(circuit_builder, private_input, n_factors);
...
pub use recursive_product_circuit::recursive_product_circuit;
mod recursive_product_circuit {
    ...
    impl RecursiveProdCircuitData {
        fn product_circuit_data(
            ...
            PRODUCT_CIRCUIT_DATA.get_or_init(|| {
                ...
                let inner_proof_verifier_target = circuit_builder
                    .constant_verifier_data(&self.recursive_circuit_data.verifier_only);
    ...
    fn build_recursive_product_circuit(
        ...
        let const_chunk_size =
            circuit_builder.constant(BaseField::from_canonical_usize(CHUNK_SIZE));
        ...
        let first_factor_after_chunk =
            circuit_builder.add(first_chunk_factor, number_of_chunk_factors);
        ...
        let proof_target_circuit_data = if remaining_bootstrap_steps == 0 {
            ...
            let num_cyclic_proof_public_inputs =
                4 + 4 * circuit_config.fri_config.num_cap_elements();
        ...
        if total_bootstrap_steps != remaining_bootstrap_steps {
            ...
            let mock_verifier_data = circuit_builder
                .add_virtual_verifier_data(circuit_builder.config.fri_config.cap_height);
    ...
    for (first_factor, n_factors) in test_round_parameters {
        ...
        let expected_product = (0..n_factors)
            .fold(
                ...
                |(prod, next_factor), _iter_nr| (prod * next_factor, next_factor + BaseField::ONE),
            )
            .0;
